...
Page({
    data: {
      ...
      // 计算结果
      resistorPairs: [] as {R1: number, Rf: number, error: number, gain: number}[],
      showNoResult: false,
      isCalculating: false,
      progress: 0,
    ...
    calculateResistors() {
      ...
      for (const R1 of allResistors) {
        ...
        // 计算实际增益
        const actualGain = 1 + (closestRf / R1);
        const actualUO = actualGain * UI_std;
        
        // 计算误差百分比
        const error = Number((Math.abs(actualUO - UO_std) / UO_std * 100).toFixed(2));
